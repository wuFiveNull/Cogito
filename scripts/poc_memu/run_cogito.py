...
def main():
    ...
    parser.add_argument("--output", default="scripts/poc_memu/results/cogito_results.json",
                        help="结果输出路径")
    ...
    if args.export:
        out = [{"doc_id": d.doc_id, "content": d.content,
                "expected_queries": d.expected_queries} for d in dataset]
        Path(args.export).parent.mkdir(parents=True, exist_ok=True)
        Path(args.export).write_text(json.dumps(out, ensure_ascii=False, indent=2),
                                     encoding="utf-8")
    ...
    try:
        ...
        latency = retrieval_latency(
            backend.retrieve, first_doc.expected_queries[0], top_k=args.top_k,
        )
