...
当前基线（截至 PLAN-09 M0）：**Ruff 0 errors，compileall clean**。
...
| Stub / OpenAI-compatible Provider | ✅ 完成 | 流式 + 非流式调用；`run_stream` 增量生成；缺省配置自动降级为 Stub |
...
| LangBot Bridge | ✅ API 已实现 | `BridgeServer._handle_delivery()` 尚仅校验 DTO + 返回 accepted；真实平台操作与 DeliveryService 集成见 PLAN-09/M5 |
