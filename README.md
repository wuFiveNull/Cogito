...
### 通过 CLI 或公开 Python API 运行

仓库提供薄 CLI 启动器；核心装配和业务能力仍由
`cogito.application.RuntimeApplication` 公开 API 承载：

```powershell
python -m cogito doctor
python -m cogito run
...
推荐通过 CLI 启动（会在同一 event loop 内管理 Web Server、Worker 与优雅停机）：

```powershell
python -m cogito serve
```

需要嵌入其他进程时，也可以通过公开 Python API 装配：
...
当前基线（2026-07-15）：**Ruff 0 errors，compileall clean，Pytest 全绿（1542 passed）**。
...
| CLI | ✅ 薄启动器 | `config/info/tools/doctor/mcp/run/serve`；核心能力仍由 `RuntimeApplication` 提供 |
