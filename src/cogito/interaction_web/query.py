...
from cogito.contracts.event_query import EventCursorError, EventPayloadUnavailableError
...
@router.get("/event-timelines")
def get_event_timeline(
    session_id: str,
    limit: int = Query(500, ge=1, le=500),
    deps: CommandDeps = Depends(get_command_deps),
) -> dict:
    return _svc(deps).event_timeline(session_id=session_id, limit=limit)
...
@router.get("/event-traces/{trace_id}")
def get_event_trace(trace_id: str, deps: CommandDeps = Depends(get_command_deps)) -> dict:
...
def proactive_status(deps: CommandDeps = Depends(get_command_deps)) -> dict:
    ...
    mock_client = manager.get_client("proactive_mock") if manager is not None else None
    connected = bool(client is not None and client.connected)
    mock_connected = bool(mock_client is not None and mock_client.connected)
    out["fetch_available"] = bool(out["enabled"] and connected)
    out["mock_available"] = bool(out["enabled"] and mock_connected)
    if not out["enabled"]:
    ...
    if not out["enabled"]:
        out["mock_unavailable_reason"] = "主动系统未启用"
    elif not mock_connected:
        out["mock_unavailable_reason"] = "主动投递 Mock MCP 未连接（请重启服务加载配置）"
    else:
        out["mock_unavailable_reason"] = ""
    return out
...
def list_events(
    ...
    try:
        return _svc(deps).list_canonical_events(
            limit=limit,
            cursor=cursor,
            before=before,
...
def get_event_payload(
    event_id: str,
    x_cogito_payload_token: str | None = Header(default=None),
    deps: CommandDeps = Depends(get_command_deps),
) -> Response:
    expected = deps.config.interaction.payload_access_token
    if not expected or not x_cogito_payload_token or not hmac.compare_digest(
        expected, x_cogito_payload_token
    ):
        raise HTTPException(status_code=403, detail="payload access is not authorized")
    try:
        out = _svc(deps).read_event_payload(event_id)
    except EventPayloadUnavailableError as exc:
