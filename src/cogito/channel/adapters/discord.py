...
class VoiceConnectionManager:
    ...
    def __init__(self, bot: discord.Client, logger: EventLogger):
        ...
        self.bot = bot
        self.logger = logger
        self.connections: dict[int, VoiceConnectionInfo] = {}
    ...
    async def get_voice_client(self, guild_id: int) -> discord.VoiceClient | None:
        """
        获取语音客户端
    ...
    async def get_connection_status(self, guild_id: int) -> dict | None:
        """
        获取连接状态信息
    ...
    async def get_voice_channel_info(self, guild_id: int, channel_id: int) -> dict | None:
        """
        获取语音频道信息
...
class DiscordAdapter(abstract_platform_adapter.AbstractMessagePlatformAdapter):
    ...
    async def get_voice_connection_status(self, guild_id: int) -> dict | None:
        """
        获取语音连接详细状态
    ...
    async def list_active_voice_connections(self) -> list[dict]:
        """
        列出所有活跃的语音连接
