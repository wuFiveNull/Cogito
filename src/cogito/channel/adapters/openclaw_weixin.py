...
from cogito.channel.clients.openclaw_weixin_api.client import (
    DEFAULT_BASE_URL,
    SESSION_EXPIRED_ERRCODE,
    OpenClawWeixinClient,
)
from cogito.channel.clients.openclaw_weixin_api.types import (
    MessageItem,
    WeixinMessage,
)
...
class OpenClawWeixinEventConverter(abstract_platform_adapter.AbstractEventConverter):
    ...
    @staticmethod
    async def target2yiri(msg: WeixinMessage) -> platform_events.MessageEvent | None:
...
class OpenClawWeixinAdapter(abstract_platform_adapter.AbstractMessagePlatformAdapter):
    ...
    _poll_task: asyncio.Task | None = pydantic.PrivateAttr(default=None)
    _bot_uuid: str | None = pydantic.PrivateAttr(default=None)
    ...
    async def _persist_config(self) -> None:
        """Persist refreshed login credentials without LangBot's database layer.

        Set ``token-file`` in the adapter config to store the updated token/base URL
        as JSON. If it is omitted, credentials remain in memory exactly as they do
        before LangBot's persistence manager writes them.
        """
        token_file = self.config.get('token-file') or self.config.get('token_file')
        if not token_file:
            return
        try:
            import json
    ...
    @staticmethod
    async def _get_component_bytes(component: platform_message.MessageComponent) -> bytes | None:
