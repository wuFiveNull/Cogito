...
def _get_group_name_placeholder(group_id: int | str) -> str:
...
class AiocqhttpEventConverter(abstract_platform_adapter.AbstractEventConverter):
    def __init__(self):
        self._group_name_cache: dict[int | str, tuple[str, float]] = {}
        self._group_name_negative_cache: dict[int | str, float] = {}
        self._group_member_info_cache: dict[
            tuple[int | str, int | str], tuple[dict, float]
        ] = {}
        self._group_member_info_negative_cache: dict[tuple[int | str, int | str], float] = {}

    @staticmethod
    async def yiri2target(event: platform_events.MessageEvent, bot_account_id: int):
        return event.source_platform_object

    async def _get_group_name(self, group_id: int | str, bot=None) -> str:
        now = time.monotonic()
...
class AiocqhttpAdapter(abstract_platform_adapter.AbstractMessagePlatformAdapter):
    ...
    on_websocket_connection_event_cache: list[typing.Callable[[aiocqhttp.Event], None]] = []
