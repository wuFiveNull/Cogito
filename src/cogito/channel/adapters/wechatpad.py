...
from cogito.channel.clients.wechatpad_api.client import WeChatPadClient
...
class WeChatPadMessageConverter(abstract_platform_adapter.AbstractMessageConverter):
    ...
    async def _handler_text(self, message: dict | None, content_no_preifx: str) -> platform_message.MessageChain:
        """处理文本消息 (msg_type=1)"""
    ...
    async def _handler_image(self, message: dict | None, content_no_preifx: str) -> platform_message.MessageChain:
        """处理图像消息 (msg_type=3)"""
        try:
    ...
    async def _handler_voice(self, message: dict | None, content_no_preifx: str) -> platform_message.MessageChain:
        """处理语音消息 (msg_type=34)"""
        message_List = []
        try:
    ...
    async def _handler_compound(self, message: dict | None, content_no_preifx: str) -> platform_message.MessageChain:
    ...
    async def _handler_compound_quote(
        self, message: dict | None, xml_data: ET.Element
    ) -> platform_message.MessageChain:
        """处理引用消息 (data_type=57)"""
        message_list = []
    ...
    async def _handler_default(self, message: dict | None, content_no_preifx: str) -> platform_message.MessageChain:
        """处理未知消息类型"""
        if message:
            msg_type = message['msg_type']
        else:
    ...
    def _handler_compound_unsupported(
        self, message: dict, xml_data: str, text: str | None = None
    ) -> platform_message.MessageChain:
    ...
    def _extract_content_and_sender(self, raw_content: str) -> tuple[str, str | None]:
        try:
