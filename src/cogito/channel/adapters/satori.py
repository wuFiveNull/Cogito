...
class SatoriEventConverter(abstract_platform_adapter.AbstractEventConverter):
    ...
    ) -> platform_events.MessageEvent | None:
...
class SatoriAdapter(abstract_platform_adapter.AbstractMessagePlatformAdapter):
    ...
    ws: typing.Any | None = pydantic.Field(exclude=True, default=None)
    session: aiohttp.ClientSession | None = pydantic.Field(exclude=True, default=None)
    running: bool = pydantic.Field(exclude=True, default=False)
    sequence: int = pydantic.Field(exclude=True, default=0)
    logins: list[dict] = pydantic.Field(exclude=True, default_factory=list)
    ready_received: bool = pydantic.Field(exclude=True, default=False)
    heartbeat_task: asyncio.Task | None = pydantic.Field(exclude=True, default=None)
    listeners: dict[type, typing.Callable] = pydantic.Field(exclude=True, default_factory=dict)
