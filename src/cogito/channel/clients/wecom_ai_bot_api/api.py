...
from cogito.channel.clients.wecom_ai_bot_api import wecombotevent
from cogito.channel.clients.wecom_ai_bot_api.WXBizMsgCrypt3 import WXBizMsgCrypt
...
class StreamSessionManager:
    ...
    def get_stream_id_by_msg(self, msg_id: str) -> str | None:
        if not msg_id:
            return None
    ...
    def get_session_by_feedback_id(self, feedback_id: str) -> StreamSession | None:
        """根据 feedback_id 查找会话。
    ...
    async def consume(self, stream_id: str, timeout: float = 0.5) -> StreamChunk | None:
...
async def parse_wecom_bot_message(
    ...
    async def _safe_download(url: str, per_msg_aeskey: str = '') -> tuple[bytes | None, str | None]:
    ...
    async def _safe_download_as_data_uri(url: str, per_msg_aeskey: str = '') -> str | None:
...
class WecomBotClient:
    def __init__(self, Token: str, EnCodingAESKey: str, Corpid: str, logger: EventLogger, unified_mode: bool = False):
        ...
        self._feedback_callback: Callable | None = None
    ...
    def _build_stream_payload(
        stream_id: str, content: str, finish: bool, feedback_id: str | None = None
    ) -> dict[str, Any]:
        """按照企业微信协议拼装返回报文。
