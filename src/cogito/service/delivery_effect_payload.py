...
_SCHEMA = "cogito.delivery-effect.v2"
...
def load_delivery_effect_payload(store: PayloadStore, payload_ref: str) -> DeliveryEffectPayload:
    ...
    if content is not None and not isinstance(content, str):
        raise ValueError("delivery effect content must be a string or null")
    ...
    return DeliveryEffectPayload(
        ...
        # v1 payloads are retained for audit/recovery but cannot be sent by
        # the canonical executor: they contain only a legacy Core reference.
        content=content,
