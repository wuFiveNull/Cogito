...
def _cadence(**over) -> ProactiveCadenceConfig:
    base = dict(
        ...
        high_energy_interval_seconds=60,
        medium_energy_interval_seconds=240,
        low_energy_interval_seconds=480,
        jitter_ratio=0.10,
    )
    base.update(over)
...
class TestComputeInterval:
    ...
    def test_clamps_to_min(self):
        cfg = _cadence(
            ...
            max_interval_seconds=1800,
            high_energy_interval_seconds=10,
            jitter_ratio=0.0,
        )
    ...
    def test_clamps_to_max(self):
        cfg = _cadence(
            ...
            low_energy_interval_seconds=5000,
            jitter_ratio=0.0,
        )
    ...
    def test_jitter_in_range(self):
        cfg = (
            _cadence(jitter_ratio=0.10, jitter_applied=True)
            if False
            else _cadence(jitter_ratio=0.10)
        )
...
class TestSchedulerCadence:
    def test_first_tick_creates_task_and_persists_next(self, memory_db):
        ...
        assert len(tasks) == 1
        state = conn.execute(
            "SELECT next_eval_at, interval_s, energy_band FROM proactive_cadence_state WHERE id=1"
        ).fetchone()
    ...
    def test_tick_after_due_fires_again(self, memory_db):
        ...
        sched = TaskListScheduler(
            conn,
            clock=clock,
            proactive_config=cfg,
        )
        first = sched.tick_proactive_evaluate()
        assert len(first) == 1
        # 推进超过 interval_s
        state = conn.execute("SELECT interval_s FROM proactive_cadence_state WHERE id=1").fetchone()
