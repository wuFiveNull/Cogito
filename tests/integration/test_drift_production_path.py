...
def test_admission_persists_real_skill_and_runner_executes():
    ...
    task = Task(task_id=task_id, task_type="drift.run", status=TaskStatus.running)
    result = handle_drift_run(task, _Ctx(conn))
    assert "unknown skill" not in result, f"Runner 不应该把已选 Skill 当 unknown 处理：{result}"
    assert "completed" in result, f"read-only Skill 应正常 completed：{result}"

    steps_row = conn.execute(
        "SELECT steps_taken FROM drift_runs WHERE drift_run_id=?",
        (run_id,),
    ).fetchone()
    assert steps_row["steps_taken"] >= 1, "真实 Skill 至少执行了 1 步"
...
def test_worker_claim_binds_attempt_id_to_checkpoint():
    ...
    ck = conn.execute(
        ...
        "WHERE drift_run_id=? ORDER BY created_at DESC LIMIT 1",
        (run_id,),
    ).fetchone()
    ...
    att_row = conn.execute(
        "SELECT checkpoint_ref FROM task_attempts WHERE task_attempt_id=?",
...
def test_pause_creates_followup_and_resume_runs_from_paused_step():
    ...
    conn.execute(
        ...
        ("t-base", "drift.run", "running", 5, "id-p02", int(time.time() * 1000)),
    )
    conn.execute(
    ...
    task_obj = Task(task_id="t-base", task_type="drift.run", status=TaskStatus.running)
    ...
    run = conn.execute(
        ...
        "FROM drift_runs WHERE drift_run_id='dr-p02'"
    ).fetchone()
    ...
    followup = conn.execute(
        ...
        "WHERE origin='drift-resume' AND task_id!='t-base'"
    ).fetchone()
    ...
    run2 = conn.execute(
        "SELECT budget_used_json, steps_taken FROM drift_runs WHERE drift_run_id='dr-p02'"
    ).fetchone()
    budget = json.loads(run2["budget_used_json"])
    assert budget.get("tool_calls", 0) == 2, f"跨 Attempt 预算应精确为 2（无双重累计）：{budget}"
    assert run2["steps_taken"] == 2, f"跨 Attempt 步数应精确为 2：{run2['steps_taken']}"
