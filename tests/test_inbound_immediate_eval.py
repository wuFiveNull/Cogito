...
def test_inbound_creates_immediate_eval_task():
    ...
    rows = conn.execute(
        "SELECT event_id, event_type, aggregate_id FROM outbox_events WHERE event_type='InboundMessageAccepted'"
    ).fetchall()
...
def test_idempotent_per_turn_per_day():
    ...
    rows = conn.execute(
        "SELECT event_id FROM outbox_events WHERE event_type='InboundMessageAccepted'"
    ).fetchone()
    event_id = rows[0]
    consumer = InboundImmediateEvalConsumer(default_principal_id="owner")
    lease = __import__("cogito.service.outbox_worker", fromlist=["OutboxLease"]).OutboxLease(
