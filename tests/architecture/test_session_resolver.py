...
def test_context_item_has_score_and_retrieval_path() -> None:
    item = ContextItem(
        item_type="memory", item_id="m1", source="sess-1",
        score=0.85, retrieval_path="keyword+vector",
    )
...
def test_resolver_reuses_existing_session(db: Any) -> None:
    ...
    first = resolver.resolve(
        channel_type="web", channel_instance_id="web-1",
        conversation_ref="conv-1", principal_id="owner",
    )
...
def test_resolver_isolates_by_principal(db: Any) -> None:
    ...
    a = resolver.resolve(
        channel_type="web", channel_instance_id="web-1",
        conversation_ref="grp-1", principal_id="user-a",
    )
    b = resolver.resolve(
        channel_type="web", channel_instance_id="web-1",
        conversation_ref="grp-1", principal_id="user-b",
    )
