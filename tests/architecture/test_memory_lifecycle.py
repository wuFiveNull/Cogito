...
def test_decay_reduces_weight(db: Any) -> None:
    ...
    repo = __import__("cogito.store.memory_repo", fromlist=["MemoryRepository"]).MemoryRepository(
        db
    )
    updated = repo.apply_decay()
    assert updated >= 1
    row = db.execute(
        "SELECT retrieval_weight FROM memory_items WHERE memory_id=?", (mid,)
    ).fetchone()
...
def test_fact_decays_slower_than_episode(db: Any) -> None:
    ...
    old = (
        __import__("datetime").datetime.now(__import__("datetime").UTC) - timedelta(days=30)
    ).isoformat()
    ...
    repo.apply_decay()
    fact_w = db.execute(
        "SELECT retrieval_weight FROM memory_items WHERE memory_id=?", (fact_id,)
    ).fetchone()[0]
    ep_w = db.execute(
        "SELECT retrieval_weight FROM memory_items WHERE memory_id=?", (ep_id,)
    ).fetchone()[0]
