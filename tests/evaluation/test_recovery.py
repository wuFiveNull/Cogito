...
class TestRecovery:
    def test_fts_corrupted_rebuild(self, db, svc):
        ...
        svc.remember(kind="fact", subject="r", predicate="x",
    ...
    def test_source_concurrent_delete_ingest(self, db):
        ...
        sync_resource(db, stable_source_id="concurrent-1", content_hash="v1",
                      raw_text="# T\n\n内容。", principal_id="owner")
        # 删除后立即重新同步
        delete_resource(db, stable_source_id="concurrent-1", principal_id="owner")
        rid2 = sync_resource(db, stable_source_id="concurrent-1", content_hash="v2",
                             raw_text="# T\n\n新内容。", principal_id="owner")
        # 新版本可检索
