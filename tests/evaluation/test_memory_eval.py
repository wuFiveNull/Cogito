...
class TestCrossSessionRecall:
    def test_fact_recalled_across_sessions(self, svc):
        ...
        svc.remember(
            kind="fact", subject="user", predicate="city", value="北京", principal_id="owner"
        )
...
class TestSupersede:
    def test_preference_update_supersedes(self, svc):
        svc.remember(
            kind="preference", subject="user", predicate="theme", value="dark", principal_id="owner"
        )
        svc.remember(
            kind="preference",
            subject="user",
...
class TestExactRetrieval:
    def test_id_date_exact_match(self, svc):
        """ID/日期/专名精确检索。"""
        svc.remember(
            kind="fact", subject="order", predicate="date", value="2026-07-11", principal_id="owner"
        )
...
class TestGoalStatus:
    def test_active_goal_recalled(self, svc):
        ...
        results = svc.retrieve(principal_id="owner", query="迁移", kinds=["goal"])
        assert len(results) >= 1
...
class TestDeleteNoResurrection:
    def test_forget_then_rebuild_no_resurrect(self, db, svc):
        ...
        mem = svc.remember(
            kind="fact", subject="tmp", predicate="x", value="v", principal_id="owner"
        )
...
class TestNoWrongReinforcement:
    def test_exposure_does_not_reinforce(self, svc):
        ...
        mem = svc.remember(
            kind="fact", subject="test", predicate="x", value="v", principal_id="owner"
        )
        # 召回前 reinforcement
