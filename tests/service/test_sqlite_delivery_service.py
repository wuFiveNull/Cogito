...
class TestEnqueue:
    ...
    async def test_enqueue_with_idempotency_returns_existing(self, conn, clock) -> None:
        ...
        r1 = await svc.enqueue(DeliveryRequest(
            target={}, content_ref="x", idempotency_key="idem-1",
        ))
        r2 = await svc.enqueue(DeliveryRequest(
            target={}, content_ref="y", idempotency_key="idem-1",
        ))
...
class TestDeliverFlow:
    async def test_deliver_success_creates_confirmed_receipt(self, conn, clock) -> None:
        ...
        svc = SqliteDeliveryService(conn, gw, clock=clock)
        ref = await svc.enqueue(DeliveryRequest(
            target={"channel": "qq"}, content_ref="hello",
        ))
